...
pm.test("Cards were dealt (9 per player)", function () {
    var jsonData = pm.response.json();
    var player = jsonData.gameState.players[0];
    pm.expect(player.cardCount).to.eql(9);
});

pm.test("Deck has remaining cards", function () {
    var jsonData = pm.response.json();
    // 104 cards - 18 dealt = 86 remaining
    pm.expect(jsonData.gameState.deck.length).to.eql(86);
});
```
...
3. **Verificar estado**
   ...
   - Verificar que las cartas fueron repartidas (9 por jugador: 3 Mano, 3 Visibles, 3 Ocultas)
